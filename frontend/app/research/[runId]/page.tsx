...
export default function ResearchRunPage() {
  ...
  return (
    <main className="page-shell research-run-shell">
      <header className="research-run-header"><div><Link className="back-link" href="/research">← New research question</Link><div className="eyebrow">RESEARCH RUN / {payload.run.id}</div><h1>{payload.run.research_question}</h1></div><div className="run-status" data-status={payload.run.status}>{payload.run.status}</div></header>
